 ...
 */
export function is(val: unknown, type: JsTypes): boolean {
 ...
 * }
 * if (isFunction(a)) {
 *  console.log(a());// a 在这个条件分支被保护为a: Function
 * } else {
 *  console.log(a);// a 在这个条件分支被保护为a: string
 * }
 * @param  {any} val 检测对象
 ...
 *
 * @param  {any} val 检测对象
 * @returns {boolean} val is StringNumber
 */
...
export function hasOwnProperty<T, U = keyof T>(target: T, key: unknown): key is U {
  return Object.prototype.hasOwnProperty.call(target, key as PropertyKey)
}
/**
 * 对象是否有同属性
 *
